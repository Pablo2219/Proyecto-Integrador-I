...
def login(datos:LoginRequest,db:Session=Depends(get_db)):
    try:return AuthService(db).login(datos.usuario,datos.contrasena)
    except ValueError as error:raise HTTPException(status_code=401,detail=str(error))
@router.post('/registro',response_model=AuthResponse,status_code=201)
def registro(datos:RegisterRequest,db:Session=Depends(get_db)):
    try:return AuthService(db).registrar(datos)
    except ValueError as error:raise HTTPException(status_code=400,detail=str(error))
@router.get('/me',response_model=UsuarioResponse)
def mi_cuenta(usuario=Depends(get_current_user),db:Session=Depends(get_db)):return AuthService(db).usuario_response(usuario)
...
def solicitar_restablecimiento(datos:PasswordRecoveryRequest,db:Session=Depends(get_db)):
    try:return AuthService(db).solicitar_restauracion(datos)
    except ValueError as error:raise HTTPException(status_code=400,detail=str(error))
@router.post('/restablecer-contrasena')
def restablecer_contrasena(datos:PasswordResetRequest,db:Session=Depends(get_db)):
    try:return AuthService(db).restablecer(datos)
    except ValueError as error:raise HTTPException(status_code=400,detail=str(error))
@router.put('/cambiar-contrasena')
def cambiar_contrasena(datos:PasswordChangeRequest,usuario=Depends(get_current_user),db:Session=Depends(get_db)):
    try:return AuthService(db).cambiar_contrasena(usuario,datos)
    except ValueError as error:raise HTTPException(status_code=400,detail=str(error))
@router.post('/logout')
def logout():return {'mensaje':'Sesión cerrada. El token debe eliminarse del cliente.'}
