...
class Usuario(Base):
    ...
    idRol: Mapped[int] = mapped_column(
        ...
        ForeignKey("Rol.idRol", onupdate="CASCADE", ondelete="RESTRICT"),
        nullable=False,
    )
    idCliente: Mapped[Optional[int]] = mapped_column(
        BIGINT(unsigned=True),
        ForeignKey("Cliente.idCliente", onupdate="CASCADE", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    estado: Mapped[str] = mapped_column(
        SqlEnum("ACTIVO", "INACTIVO", "BLOQUEADO"),
        nullable=False,
        default="ACTIVO",
    )
